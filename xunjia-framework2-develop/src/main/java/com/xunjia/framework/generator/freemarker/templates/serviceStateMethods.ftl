	...
	public ResponseData<Boolean> updateEnableState(${enableProp.type} ${enableProp.propName}, String[] ids){
		...
		try {
			...
			LOGGER.error("${entity.entityName?uncap_first}Service.updateEnableState方法异常。", e);
	...
	public List<${entity.entityName}> findEnable${entity.entityName?cap_first}s(){
		return ${entity.entityName?uncap_first}Repo.findBy${enableProp.propName?cap_first}(
			<#switch enableProp.type>
