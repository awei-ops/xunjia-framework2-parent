...
<html xmlns="http://www.w3.org/1999/xhtml"
	...
	xmlns:bf="http://www.thymeleaf.org">
	

<div class="table-container">
	<form id="edit-${entity.entityName?uncap_first}-form" method="post" enctype="${encType}">
		<input type="hidden" id="${entity.entityName?uncap_first}-id" name="id" />
		<#if enableProp??>
			<input type="hidden" id="${entity.entityName?uncap_first}-${enableProp.propName}" name="${enableProp.propName}" />
		</#if>
					...
					<tr>
						...
						<td>
							<#switch prop.controlType>
								<#case "文本">
									<input class="easyui-textbox" id="${entity.entityName?uncap_first}-${prop.propName}" name="${prop.propName}" data-options="<#if prop.requiredFlag == 1>required:true,</#if>width:240" />
									<#break>
								<#case "数字">
									...
									<#break>
								<#case "日期">
									<input class="easyui-datebox" id="${entity.entityName?uncap_first}-${prop.propName}" name="${prop.propName}" data-options="<#if prop.requiredFlag == 1>required:true,</#if>editable:false,width:240" />
									<#break>
								<#case "下拉框">
