	...
	public Page<${entity.entityName}> find${entity.entityName?cap_first}s(
		...
		<#list entity.properties as p>
			<#if p.orderBy?? && p.orderBy?length gt 2>
				<#assign orderExist = "true">
				<#break>
			</#if>
		</#list>
		
		<#if orderExist?? && orderExist == "true">
		...
		try {
			...
			LOGGER.error("${entity.entityName?uncap_first}Service.find${entity.entityName?cap_first}s方法异常。", e);
		}
		return pageData;
	}