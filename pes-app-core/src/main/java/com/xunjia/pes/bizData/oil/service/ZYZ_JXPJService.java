...
@Transactional
@Slf4j
public class ZYZ_JXPJService {
    @Autowired
    private DMGC_Y_ZYZService yZyzService;
    ...
    @Autowired
    @Lazy(true)
    private DMGC_Y_D_ZYZService zyzService;
    ...
    public List<Station_pj> getJXPJ(String cycle, String assessmentDate) {
        ...
        try {
            List<DMGC_Y_ZYZ> zyzList = yZyzService.getAll();
            List<String> zyzIds = zyzList.stream().map(BaseEntity::getEventId).distinct().collect(Collectors.toList());
//            List<DMGC_Y_D_SYB> sybList = sybService.getAssessmentNoPage(cycle, assessmentDate);
//            sybList = sybList.stream().filter(c -> zyzIds.contains(c.getSszkEventId())).collect(Collectors.toList());
            List<DMGC_Y_D_SYB> yDSybList = sybService.getEffectiveDataOfDay(zyzIds, assessmentDate);
//            List<DMGC_Y_D_CSB> yDCsbList = csbService.getAssessmentNoPage(cycle, assessmentDate);
//            yDCsbList = yDCsbList.stream().filter(c -> zyzIds.contains(c.getSszkEventId())).collect(Collectors.toList());
            List<DMGC_Y_D_CSB> yDCsbList = csbService.getEffectiveDataOfDay(zyzIds, assessmentDate);
//            List<DMGC_Y_D_JRL> yDJrlList = jrlService.getAssessmentNoPage(cycle, assessmentDate);
//            yDJrlList = yDJrlList.stream().filter(c -> zyzIds.contains(c.getSszkEventId())).collect(Collectors.toList());
            List<DMGC_Y_D_JRL> yDJrlList = jrlService.getEffectiveDataOfDay(zyzIds, assessmentDate);
//            List<DMGC_Y_D_ZYZ> yDZyzList = zyzService.getAssessmentNoPage(cycle, assessmentDate);
            List<DMGC_Y_D_ZYZ> yDZyzList = zyzService.getEffectiveDataOfDay(assessmentDate);
            ...
            List<DynamicWeightResult> dynamicWeightResultList = dynamicWeights.getZyzWeightMap(zyzList, dmgc_jrlList, yDJrlList,
            ...
            if (yDSybList.size() != 0) {
//                List<String> jbIds = yDSybList.stream().map(c -> c.getJbEventId()).distinct().collect(Collectors.toList());
//                List<DMGC_Y_JB> jbList = jbService.getByEventIds(jbIds);
                for (DMGC_Y_D_SYB param : yDSybList) {
                    ...
                    for (Indicators indicators : jbIndicators) {
                        ...
                        zyzJxpj.setStationId(param.getSszkEventId());
                        zyzJxpj.setStationName(param.getZm());
                        zyzJxpj.setEquipmentId(param.getJbEventId());
...
//                        zyzJxpj.setLevelTwo_weight(levll2.getWeight());
                        zyzJxpj.setLevelTwo_weight(dynamicWeightResult.getLevel2Weight());

//                        Indicators levll3 = indicatorsList.stream().filter(c -> c.getLevelCode().equals("level3") && c.getItemCode().equals("sybjszb")).findFirst().get();
//                        zyzJxpj.setLevelThree_name(levll3.getItemName());
//                        zyzJxpj.setLevelThree_weight(levll3.getWeight());
                        zyzJxpj.setLevelThree_name(param.getEquipName());
                        zyzJxpj.setLevelThree_weight(dynamicWeightResult.getLevel3Weight());

                        zyzJxpj.setLevelFour_name(param.getEquipName() + param.getSybbh());
...
//                        zyzJxpj.setLevelFour_weight(Calculation.getDivisionResult(djgl_current, djgl_all));
                        zyzJxpj.setLevelFour_weight(dynamicWeightResult.getLevel4Weight());
                        zyzJxpj.setLevelFour_score(param.getJbScore());

                        zyzJxpj.setLevelFive_name(indicators.getItemName());
                        zyzJxpj.setLevelFive_weight(indicators.getWeight());
                        switch (indicators.getItemCode()) {
                            case "efficiency":
                                zyzJxpj.setLevelFive_score(param.getSybxlScore());
                                break;
                            case "loss":
                                zyzJxpj.setLevelFive_score(param.getJlsslScore());
                                break;
                            case "hlssl":
                                zyzJxpj.setLevelFive_score(param.getHlRateScore());
                                break;
                            case "fhl":
                                zyzJxpj.setLevelFive_score(param.getFhlScore());
                                break;
                        }
                        result.add(zyzJxpj);
                    }
            ...
            if (yDCsbList.size() != 0) {
//                List<String> jbIds = yDCsbList.stream().map(c -> c.getJbEventId()).distinct().collect(Collectors.toList());
//                List<DMGC_Y_JB> jbList = jbService.getByEventIds(jbIds);
...
//                        zyzJxpj.setLevelThree_weight(levll3.getWeight());
                        ...
                        zyzJxpj.setLevelFour_name(param.getEquipName() + param.getZnbh());
//                        double djgl_all = jbList.stream().filter(c -> c.getSszkEventId().equals(param.getSszkEventId())).mapToDouble(DMGC_Y_JB::getDjgl).sum();
//                        double djgl_current = jbList.stream().filter(c -> c.getEventId().equals(param.getJbEventId())).findFirst().get().getDjgl();
//                        zyzJxpj.setLevelFour_weight(Calculation.getDivisionResult(djgl_current, djgl_all));
                        zyzJxpj.setLevelFour_weight(dynamicWeightResult.getLevel4Weight());
                        ...
                        switch (indicators.getItemCode()) {
                            case "efficiency":
                                zyzJxpj.setLevelFive_score(param.getBxlScore());
                                break;
                            case "loss":
                                zyzJxpj.setLevelFive_score(param.getJlsslScore());
                                break;
            ...
            if (yDJrlList.size() != 0) {
//                List<String> jrlIds = yDJrlList.stream().map(c -> c.getJrlId()).distinct().collect(Collectors.toList());
//                List<DMGC_JRL> dmgc_jrlList = dmgcJrlService.getByIds(jrlIds);
                for (DMGC_Y_D_JRL param : yDJrlList) {
                    ...
                    for (Indicators indicators : jrlIndicators) {
                        Station_pj zyzJxpj = new Station_pj();
                        zyzJxpj.setStationId(param.getSszkEventId());
                        zyzJxpj.setStationName(param.getSszkName());
                        zyzJxpj.setEquipmentId(param.getJrlId());
                        zyzJxpj.setRq(sumEnd);
...
//                        Indicators levll3 = indicatorsList.stream().filter(c -> c.getLevelCode().equals("level3") && c.getItemCode().equals("csljszb")).findFirst().get();
//                        zyzJxpj.setLevelThree_name(levll3.getItemName());
//                        zyzJxpj.setLevelThree_weight(levll3.getWeight());
                        zyzJxpj.setLevelThree_name(param.getMc());
                        zyzJxpj.setLevelThree_weight(dynamicWeightResult.getLevel3Weight());

                        zyzJxpj.setLevelFour_name(param.getMc() + param.getZnbh());
...
//                        zyzJxpj.setLevelFour_weight(Calculation.getDivisionResult(djgl_current, djgl_all));
                        zyzJxpj.setLevelFour_weight(dynamicWeightResult.getLevel4Weight());
                        zyzJxpj.setLevelFour_score(param.getJxScore());

                        zyzJxpj.setLevelFive_name(indicators.getItemName());
                        zyzJxpj.setLevelFive_weight(indicators.getWeight());
                        switch (indicators.getItemCode()) {
                            case "jrlxl":
                                zyzJxpj.setLevelFive_score(param.getLxScore());
                                break;
                            case "pywd":
                                zyzJxpj.setLevelFive_score(param.getYqwdScore());
                                break;
                            case "kqxs":
                                zyzJxpj.setLevelFive_score(param.getKqxsScore());
                                break;
                            case "rfh":
                                zyzJxpj.setLevelFive_score(param.getRfhScore());
                                break;
                            case "ltbmwd":
                                zyzJxpj.setLevelFive_score(param.getLtwbmwdScore());
                                break;
                        }
                        result.add(zyzJxpj);
                    }
            ...
            for (DMGC_Y_D_ZYZ param : yDZyzList) {
                Indicators levll2 = indicatorsList.stream().filter(c -> c.getLevelCode().equals("level2") && c.getItemCode().equals("zyzzhdh")).findFirst().get();
                Station_pj zyzJxpj = new Station_pj();
                zyzJxpj.setStationId(param.getZkEventId());
                ...
                zyzJxpj.setLevelOne_name(levll1.getItemName());
                if (result.stream().noneMatch(c -> c.getStationId().equals(param.getZkEventId()) && c.getEquipmentId() != null)) {
                    zyzJxpj.setLevelOne_weight(Double.parseDouble("1"));
                } else {
                    zyzJxpj.setLevelOne_weight(levll1.getWeight());
                }
                ...
                levll2 = indicatorsList.stream().filter(c -> c.getLevelCode().equals("level2") && c.getItemCode().equals("zyzdyhd")).findFirst().get();
                ...
                zyzJxpj.setLevelTwo_name(levll2.getItemName());
                zyzJxpj.setLevelFive_weight(levll2.getWeight());
                zyzJxpj.setLevelFive_score(param.getDyhdScore());
                result.add(zyzJxpj);

                levll2 = indicatorsList.stream().filter(c -> c.getLevelCode().equals("level2") && c.getItemCode().equals("zyzdyhq")).findFirst().get();
                zyzJxpj = new Station_pj();
                zyzJxpj.setStationId(param.getZkEventId());
                zyzJxpj.setStationName(param.getStationName());
                zyzJxpj.setRq(sumEnd);
                ...
                zyzJxpj.setLevelTwo_name(levll2.getItemName());
                zyzJxpj.setLevelFive_weight(levll2.getWeight());
                zyzJxpj.setLevelFive_score(param.getDyhqScore());
                result.add(zyzJxpj);
            }
    ...
    public Boolean updateData(String rq) {
        ...
        try {
            ...
            //转油站需要更新经济指标的分数
//            zyzService.updateData(rq);
            List<Station_pj> temp = getJXPJ("日", nowDateString);
//            List<DMGC_Y_D_ZYZ> zyzList = zyzService.getAssessmentNoPage("日", nowDateString);
            List<DMGC_Y_D_ZYZ> zyzList = zyzService.getEffectiveDataOfDay(nowDateString);
            List<DMGC_Y_D_ZYZ> updateZyz = new ArrayList<>();
            List<String> zyzIds = temp.stream().map(c -> c.getStationId()).distinct().collect(Collectors.toList());
            for (String zyzId : zyzIds) {
                Station_pj myStation = temp.stream().filter(c -> c.getStationId().equals(zyzId)).collect(Collectors.toList()).get(0);
                Optional<DMGC_Y_D_ZYZ> optional = zyzList.stream().filter(c -> c.getZkEventId().equals(zyzId)).findFirst();
            ...
            if (updateZyz.size() != 0) {
                zyzService.updateData(updateZyz);
            }
        } catch (Exception ex) {
            String err = ex.getMessage();
            result = false;
    ...
    public List<Station_pj> getRelativeJXPJ(String cycle, String zid, String queryDateString) {
        ...
        try {
            ...
            List<DMGC_Y_D_ZYZ> myDatas = zyzService.getDataForPortrait(zid, queryStart, queryEnd);
            List<String> stationIds = myDatas.stream().map(DMGC_Y_D_ZYZ::getZkEventId).distinct().collect(Collectors.toList());
            if (!myDatas.isEmpty()) {
                for (String stationId : stationIds) {
                    Station_pj stationPj = new Station_pj();
                    stationPj.setRq(queryStart);
                    stationPj.setStationId(stationId);
                    stationPj.setStationName(myDatas.stream().filter(c -> c.getZkEventId().equals(stationId)).collect(Collectors.toList()).get(0).getStationName());
                    long count = 1;
                    double curScore = 0;
                    ...
                    curScore = myDatas.stream().filter(c -> c.getZkEventId().equals(stationId) && c.getRq().compareTo(queryStart) >= 0 && c.getRq().compareTo(nextStart) < 0 && c.getJxpjScore() != null)
                            .mapToDouble(DMGC_Y_D_ZYZ::getJxpjScore).sum();
    ...
    public List<Station_pj> getSinglePortraitJXPJ(String cycle, String zid, String queryDateString) {
        ...
        try {
            ...
            List<DMGC_Y_D_ZYZ> myDatas = zyzService.getDataForPortrait(zid, preStart, preEnd);
            myDatas.addAll(zyzService.getDataForPortrait(zid, queryStart, queryEnd));
            List<String> stationIds = myDatas.stream().map(DMGC_Y_D_ZYZ::getZkEventId).distinct().collect(Collectors.toList());
    ...
    public List<Station_pj> getSingleComprehensiveJXPJ(String cycle, String zid, String queryDateString) {
        ...
        try {
            ...
            if(StringUtils.isNotEmpty(zid)){
                zyzList = zyzList.stream().filter(c->c.getEventId().equals(zid)).collect(Collectors.toList());
            }
            List<MonitoringIndicatorNew> monitoringIndicatorNewList = monitoringIndicatorNewService.findByParams("纵向绩效指标要求", null);
            ...
            if (!monitoringIndicatorNewList.isEmpty() && !myDatas.isEmpty()) {
                for (DMGC_Y_ZYZ dmgcYZyz : zyzList) {
                    String stationId = dmgcYZyz.getEventId();
                    Station_pj stationPj = new Station_pj();
                    stationPj.setRq(queryStart);
                    stationPj.setStationId(stationId);
                    stationPj.setStationName(dmgcYZyz.getMc());
    ...
    public List<Station_pj> getPortraitJXPJ(String zid, String queryStart, String queryEnd) {
        ...
        try {
            ...
            List<DMGC_Y_D_ZYZ> myDatas = zyzService.getDataForPortrait(zid, preSumStart, sumEnd);
