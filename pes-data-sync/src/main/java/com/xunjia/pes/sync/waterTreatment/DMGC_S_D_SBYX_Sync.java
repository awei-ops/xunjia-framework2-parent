...
@Transactional
@Slf4j
public class DMGC_S_D_SBYX_Sync {
    @Autowired
    ...
    @Autowired
    private DMGC_S_SCLJBMapper sScljbMapper;
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        for (Date start = startTime; start.getTime() <= endTime.getTime(); start = DateUtils.addDate(start, 1)) {
            List<DMGC_S_D_SBYX> records = getRecordsInDb(start);
            ...
            if (StringUtil.isNotBlank(temp)) {
                readDataSql = "select * from A5ADMIN.DMGC_S_D_SBYX where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN) + "', 'yyyy-MM-dd HH24:mi:ss') AND EVENTID not in (" + temp + ")";
            } else {
                readDataSql = "select * from A5ADMIN.DMGC_S_D_SBYX where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN)  + "', 'yyyy-MM-dd HH24:mi:ss')";
            }
            List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);
            // 读取readDataList，构造实体
            List<DMGC_S_D_SBYX> entities = this.convertToEntity(readDataList);
            // 实体批量写入
            if (!entities.isEmpty()) {
                for (DMGC_S_D_SBYX entity : entities) {
    ...
    private List<DMGC_S_D_SBYX> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_D_SBYX> entities = new ArrayList<>();
        LambdaQueryWrapper<DMGC_S_SCLJB> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_SCLJB> sScljbs = sScljbMapper.selectList(wrapper);
        Class clazz = DMGC_S_D_SBYX.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_D_SBYX newData = new DMGC_S_D_SBYX();
                for (Field field : fields) {
                    ...
                    switch (entityKey){
                        ...
                        case "sszm":
                            ...
                            List<DMGC_S_SCLJB> temp = sScljbs.stream().filter(c->c.getEventId().equals(data.get("SBID").toString())).collect(Collectors.toList());
                            if(temp.size() != 0){
                                field.set(newData,temp.get(0).getSszkmc());
                            }
                            break;
                        default:
                            key = StringUtils.humpToUnderline(field.getName()).toUpperCase();
    ...
    private List<DMGC_S_D_SBYX> getRecordsInDb(Date rq) {
        LambdaQueryWrapper<DMGC_S_D_SBYX> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(DMGC_S_D_SBYX::getRq, rq);
        List<DMGC_S_D_SBYX> result = mapper.selectList(wrapper);
        return result;
    }
}
