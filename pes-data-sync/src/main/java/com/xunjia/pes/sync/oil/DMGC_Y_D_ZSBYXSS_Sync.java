...
@Transactional
@Slf4j
public class DMGC_Y_D_ZSBYXSS_Sync {
    @Autowired
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        for (Date start = startTime; start.getTime() <= endTime.getTime(); start = DateUtils.addDate(start, 1)) {
            List<DMGC_Y_D_ZSBYXSS> records = getRecordsInDb(start);
            ...
            if (StringUtil.isNotBlank(temp)) {
                readDataSql = "select * from A5ADMIN.DMGC_Y_D_ZSBYXSS where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN) + "', 'yyyy-MM-dd HH24:mi:ss') AND EVENTID not in (" + temp + ")";
            } else {
                readDataSql = "select * from A5ADMIN.DMGC_Y_D_ZSBYXSS where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN)  + "', 'yyyy-MM-dd HH24:mi:ss')";
            }
            List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);
            // 读取readDataList，构造实体
            List<DMGC_Y_D_ZSBYXSS> entities = this.convertToEntity(readDataList);
            // 实体批量写入
            if (!entities.isEmpty()) {
                for (DMGC_Y_D_ZSBYXSS entity : entities) {
    ...
    private List<DMGC_Y_D_ZSBYXSS> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_Y_D_ZSBYXSS> entities = new ArrayList<>();
        Class clazz = DMGC_Y_D_ZSBYXSS.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_Y_D_ZSBYXSS newData = new DMGC_Y_D_ZSBYXSS();
    ...
    private List<DMGC_Y_D_ZSBYXSS> getRecordsInDb(Date rq) {
        LambdaQueryWrapper<DMGC_Y_D_ZSBYXSS> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(DMGC_Y_D_ZSBYXSS::getRq, rq);
        List<DMGC_Y_D_ZSBYXSS> result = mapper.selectList(wrapper);
        return result;
    }
}
