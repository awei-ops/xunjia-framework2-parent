...
public class DMGC_Y_JB_Sync {
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_Y_JB> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_y_jb where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_y_jb";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_Y_JB> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_Y_JB entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_Y_JB> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_Y_JB> entities = new ArrayList<>();
        Class clazz = DMGC_Y_JB.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_Y_JB newData = new DMGC_Y_JB();
    ...
    private List<DMGC_Y_JB> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_Y_JB> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_Y_JB> result = mapper.selectList(wrapper);
        return result;
    }
}
