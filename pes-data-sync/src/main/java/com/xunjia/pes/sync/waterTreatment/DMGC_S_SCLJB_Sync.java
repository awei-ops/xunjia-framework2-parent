...
@Transactional
@Slf4j
public class DMGC_S_SCLJB_Sync {
    @Autowired
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_S_SCLJB> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_s_scljb where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_s_scljb";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_S_SCLJB> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_S_SCLJB entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_S_SCLJB> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_SCLJB> entities = new ArrayList<>();
        Class clazz = DMGC_S_SCLJB.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_SCLJB newData = new DMGC_S_SCLJB();
    ...
    private List<DMGC_S_SCLJB> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_S_SCLJB> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_SCLJB> result = mapper.selectList(wrapper);
        return result;
    }
}
