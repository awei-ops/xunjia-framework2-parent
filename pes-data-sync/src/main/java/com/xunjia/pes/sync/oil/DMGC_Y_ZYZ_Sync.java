...
public class DMGC_Y_ZYZ_Sync {
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_Y_ZYZ> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_y_zyz where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_y_zyz";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_Y_ZYZ> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_Y_ZYZ entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_Y_ZYZ> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_Y_ZYZ> entities = new ArrayList<>();
        Class clazz = DMGC_Y_ZYZ.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_Y_ZYZ newData = new DMGC_Y_ZYZ();
    ...
    private List<DMGC_Y_ZYZ> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_Y_ZYZ> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_Y_ZYZ> result = mapper.selectList(wrapper);
        return result;
    }
}
