...
public class DMGC_S_JB_Sync {
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_S_JB> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_s_jb where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_s_jb";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_S_JB> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_S_JB entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_S_JB> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_JB> entities = new ArrayList<>();
        Class clazz = DMGC_S_JB.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_JB newData = new DMGC_S_JB();
    ...
    private List<DMGC_S_JB> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_S_JB> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_JB> result = mapper.selectList(wrapper);
        return result;
    }
}
