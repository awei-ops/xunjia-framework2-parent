...
public class DMGC_JRL_Sync {
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_JRL> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_jrl where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_jrl";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_JRL> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_JRL entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_JRL> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_JRL> entities = new ArrayList<>();
        Class clazz = DMGC_JRL.class;
    ...
    private List<DMGC_JRL> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_JRL> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_JRL> result = mapper.selectList(wrapper);
        return result;
    }
}
