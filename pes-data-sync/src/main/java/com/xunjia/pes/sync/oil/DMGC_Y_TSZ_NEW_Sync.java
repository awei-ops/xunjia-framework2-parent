...
@Transactional
@Slf4j
public class DMGC_Y_TSZ_NEW_Sync {
    @Autowired
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_Y_TSZ_NEW> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_y_tsz_new where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_y_tsz_new";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_Y_TSZ_NEW> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_Y_TSZ_NEW entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_Y_TSZ_NEW> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_Y_TSZ_NEW> entities = new ArrayList<>();
        Class clazz = DMGC_Y_TSZ_NEW.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_Y_TSZ_NEW newData = new DMGC_Y_TSZ_NEW();
    ...
    private List<DMGC_Y_TSZ_NEW> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_Y_TSZ_NEW> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_Y_TSZ_NEW> result = mapper.selectList(wrapper);
        return result;
    }
}
