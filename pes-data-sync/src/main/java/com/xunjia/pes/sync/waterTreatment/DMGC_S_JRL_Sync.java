...
public class DMGC_S_JRL_Sync {
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_S_JRL> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_s_jrl where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_s_jrl";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_S_JRL> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_S_JRL entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_S_JRL> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_JRL> entities = new ArrayList<>();
        Class clazz = DMGC_S_JRL.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_JRL newData = new DMGC_S_JRL();
    ...
    private List<DMGC_S_JRL> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_S_JRL> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_JRL> result = mapper.selectList(wrapper);
        return result;
    }
}
