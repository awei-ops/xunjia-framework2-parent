...
@Transactional
@Slf4j
public class DMGC_S_D_ZSZRSJ_Sync {
    @Autowired
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        for (Date start = startTime; start.getTime() <= endTime.getTime(); start = DateUtils.addDate(start, 1)) {
            List<DMGC_S_D_ZSZRSJ> records = getRecordsInDb(start);
            ...
            if (StringUtil.isNotBlank(temp)) {
                readDataSql = "select * from A5ADMIN.DMGC_S_D_ZSZRSJ where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN) + "', 'yyyy-MM-dd HH24:mi:ss') AND EVENTID not in (" + temp + ")";
            } else {
                readDataSql = "select * from A5ADMIN.DMGC_S_D_ZSZRSJ where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN)  + "', 'yyyy-MM-dd HH24:mi:ss')";
            }
            List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);
            // 读取readDataList，构造实体
            List<DMGC_S_D_ZSZRSJ> entities = this.convertToEntity(readDataList);
            // 实体批量写入
            if (!entities.isEmpty()) {
                setZMC(entities);
                for (DMGC_S_D_ZSZRSJ entity : entities) {
    ...
    private  void  setZMC( List<DMGC_S_D_ZSZRSJ> entities){
        LambdaQueryWrapper<DMGC_S_ZSZ> queryWrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_ZSZ> dmgcSZszList = zszMapper.selectList(queryWrapper);
        List<DMGC_S_D_ZSZRSJ> temp = entities.stream().filter(c->c.getZmc() == null).collect(Collectors.toList());
        for (DMGC_S_D_ZSZRSJ dmgcSDZszrsj : temp) {
            Optional<DMGC_S_ZSZ> optional = dmgcSZszList.stream().filter(c->c.getEventId().equals(dmgcSDZszrsj.getZid())).findFirst();
            optional.ifPresent(c->{
                dmgcSDZszrsj.setZmc(c.getMc());
            });
        }
    }

    private List<DMGC_S_D_ZSZRSJ> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_D_ZSZRSJ> entities = new ArrayList<>();
        Class clazz = DMGC_S_D_ZSZRSJ.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_D_ZSZRSJ newData = new DMGC_S_D_ZSZRSJ();
    ...
    private List<DMGC_S_D_ZSZRSJ> getRecordsInDb(Date rq) {
        LambdaQueryWrapper<DMGC_S_D_ZSZRSJ> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(DMGC_S_D_ZSZRSJ::getRq, rq);
        List<DMGC_S_D_ZSZRSJ> result = mapper.selectList(wrapper);
        return result;
    }
}
