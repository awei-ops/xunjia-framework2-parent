...
public class DMGC_S_ZSZ_Sync {
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        List<DMGC_S_ZSZ> records = getRecordsInDb();
        ...
        if (StringUtil.isNotBlank(temp)) {
            readDataSql = "select * from A5ADMIN.dmgc_s_zsz where EVENTID not in (" + temp + ")";
        } else {
            readDataSql = "select * from A5ADMIN.dmgc_s_zsz";
        }
        List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);

        // 读取readDataList，构造实体
        List<DMGC_S_ZSZ> entities = this.convertToEntity(readDataList);

        // 实体批量写入
        if (!entities.isEmpty()) {
            for (DMGC_S_ZSZ entity : entities) {
                mapper.insert(entity);
            }
        }
    }

    private List<DMGC_S_ZSZ> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_ZSZ> entities = new ArrayList<>();
        Class clazz = DMGC_S_ZSZ.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_ZSZ newData = new DMGC_S_ZSZ();
    ...
    private List<DMGC_S_ZSZ> getRecordsInDb() {
        LambdaQueryWrapper<DMGC_S_ZSZ> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_ZSZ> result = mapper.selectList(wrapper);
        return result;
    }
}
