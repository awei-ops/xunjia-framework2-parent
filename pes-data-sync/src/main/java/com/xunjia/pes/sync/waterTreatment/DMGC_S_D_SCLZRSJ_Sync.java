...
@Transactional
@Slf4j
public class DMGC_S_D_SCLZRSJ_Sync {
    @Autowired
    ...
    @Autowired
    private DMGC_S_SCLZMapper sSclzMapper;
    ...
    public void sync(Date startTime, Date endTime) throws IllegalAccessException {
        for (Date start = startTime; start.getTime() <= endTime.getTime(); start = DateUtils.addDate(start, 1)) {
            List<DMGC_S_D_SCLZRSJ> records = getRecordsInDb(start);
            ...
            if (StringUtil.isNotBlank(temp)) {
                readDataSql = "select * from A5ADMIN.DMGC_S_D_SCLZRSJ where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN) + "', 'yyyy-MM-dd HH24:mi:ss') AND EVENTID not in (" + temp + ")";
            } else {
                readDataSql = "select * from A5ADMIN.DMGC_S_D_SCLZRSJ where RQ=to_date('" + DateUtils.format(start,DateUtils.DATE_TIME_PATTERN)  + "', 'yyyy-MM-dd HH24:mi:ss')";
            }
            List<Map<String, Object>> readDataList = jdbcTemplate.queryForList(readDataSql);
            // 读取readDataList，构造实体
            List<DMGC_S_D_SCLZRSJ> entities = this.convertToEntity(readDataList);
            // 实体批量写入
            if (!entities.isEmpty()) {
                for (DMGC_S_D_SCLZRSJ entity : entities) {
    ...
    private List<DMGC_S_D_SCLZRSJ> convertToEntity(List<Map<String, Object>> dataList) throws IllegalAccessException {
        List<DMGC_S_D_SCLZRSJ> entities = new ArrayList<>();
        LambdaQueryWrapper<DMGC_S_SCLZ> wrapper = new LambdaQueryWrapper<>();
        List<DMGC_S_SCLZ> sclzList = sSclzMapper.selectList(wrapper);
        Class clazz = DMGC_S_D_SCLZRSJ.class;
        ...
        if (dataList.size() > 0) {
            for (Map<String, Object> data : dataList) {
                DMGC_S_D_SCLZRSJ newData = new DMGC_S_D_SCLZRSJ();
                for (Field field : fields) {
                    ...
                    switch (entityKey){
                        ...
                        case "zmc":
                            ...
                            List<DMGC_S_SCLZ> temp = sclzList.stream().filter(c->c.getEventId().equals(data.get("ZK_EVENTID").toString())).collect(Collectors.toList());
                            if(temp.size() != 0){
    ...
    private List<DMGC_S_D_SCLZRSJ> getRecordsInDb(Date rq) {
        LambdaQueryWrapper<DMGC_S_D_SCLZRSJ> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(DMGC_S_D_SCLZRSJ::getRq, rq);
        List<DMGC_S_D_SCLZRSJ> result = mapper.selectList(wrapper);
        return result;
    }
}
